...
**Environment label:** collector-misfire-demo
**Evidence source:** local_fixture: incomplete_evidence.json
...
**Result:** NEEDS HUMAN (`manual_review`)
...
- Snapshot did not include any IAM customer-managed policies. That is normal for tiny demos, but for an audit you still need to show how access is granted (inline policies, permission sets, or attached AWS managed policies gathered elsewhere).
...
**Review queue (missing data, judgement calls, softer risks):**
- Neither S3 nor RDS objects were shipped in this file, so we cannot show encryption at rest for those services. That is not a pass—it means the story is unfinished unless other evidence exists.
...
**Review queue (missing data, judgement calls, softer risks):**
- CloudTrail section is basically empty. Before anyone calls this a pass, you need at least one region worth of `DescribeTrails` / `GetTrailStatus` output in the bundle.
